...
pub fn dup(thread: &mut RefMut<Thread>) {
    ...
    let variable_slot_clone = stack_frame.operand_stack.last().clone();
    if variable_slot_type_is_kind_one(&variable_slot_clone) {
        stack_frame.operand_stack.push(variable_slot_clone);
    } else {
...
pub fn dup_x1(thread: &mut RefMut<Thread>) {
    ...
    if variable_slot_type_is_kind_one(&first) && variable_slot_type_is_kind_one(&second) {
        stack_frame.operand_stack.extend_with_slice(&[first_clone, second, first]);
    } else {
...
pub fn dup_x2(thread: &mut RefMut<Thread>) {
    ...
    if variable_slot_type_is_kind_one(&first) {
        if variable_slot_type_is_kind_one(&second) {
            let third = stack_frame.operand_stack.pop();
            if variable_slot_type_is_kind_one(&third) {
                stack_frame.operand_stack.extend_with_slice(&[first.clone(), third, second, first]);
            } else {
                panic!("dup_x2 error:  variable_slot third is {:?}", third);
            }
        } else {
            stack_frame.operand_stack.extend_with_slice(&[first.clone(), second, first]);
        }
    } else {
...
pub fn dup2(thread: &mut RefMut<Thread>) {
    ...
    if variable_slot_type_is_kind_one(&first) {
        let second = stack_frame.operand_stack.pop();
        if variable_slot_type_is_kind_one(&second) {
            stack_frame.operand_stack.extend_with_slice(&[second.clone(), first.clone(), second, first]);
        } else {
            panic!("dup2 error:  variable_slot second is {:?}", second);
        }
    } else {
        stack_frame.operand_stack.extend_with_slice(&[first.clone(), first]);
    }
}
...
pub fn dup2_x1(thread: &mut RefMut<Thread>) {
    ...
    if variable_slot_type_is_kind_one(&first) {
        ...
        if variable_slot_type_is_kind_one(&second) {
            let third = stack_frame.operand_stack.pop();
            if variable_slot_type_is_kind_one(&third) {
                stack_frame.operand_stack.extend_with_slice(&[second.clone(), first.clone(), third, second, first]);
            } else {
...
pub fn dup2_x2(thread: &mut RefMut<Thread>) {
    ...
    if variable_slot_type_is_kind_one(&first) {
        if variable_slot_type_is_kind_one(&second) {
            let third = stack_frame.operand_stack.pop();
            if variable_slot_type_is_kind_one(&third) {
                let fourth = stack_frame.operand_stack.pop();
                if variable_slot_type_is_kind_one(&fourth) {
                    // stack head 1 1 1 1 stack bottom
                    stack_frame.operand_stack.extend_with_slice(&[second.clone(), first.clone(), fourth, third, second, first]);
                } else {
