...
1. ClassLoader & ClassReader
   ...
   * ClassLoader负责类的加载流程控制和加载进来的类的存储 class_reader读取出ClassFile ->  转换生成Class ->  class_loader的class_map存储Class
2. Runtime - method_area
   * VariableSlot: 局部变量表中最基本的存储单元，称之为变量槽，32位以内的数据占用一个VariableSlot，64位类型的占用两个VariableSlot
